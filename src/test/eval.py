...
from load_model import MobilePhoneNetTR #PhoneAttributeTF #MobilePhoneNet,MobilePhoneNetTR,
...
def args():
    ...
    parser.add_argument('--file_in',type=str,dest='file_in',default='None',\
    ...
    parser.add_argument('--cuda_use', default=True, type=str2bool,
                        help='gpu run')
    parser.add_argument('--min_size',type=int,dest='min_size',default=50,\
                        help="scale img size")
    ...
    parser.add_argument('--img-path2',type=str,dest='img_path2',default="test2.jpg",\
                        help="scale img size")
    parser.add_argument('--base_dir',type=str,dest='base_dir',default="./base_dir",\
                        help="images saved dir")
    parser.add_argument('--gpu', default=None, type=str,help='which gpu to run')
    parser.add_argument('--record_file',type=str,dest='record_file',default=None,\
                        help="output saved file")
    parser.add_argument('--failed-dir',type=str,dest='failed_dir',default="./failed_dir",\
                        help="fpr saved dir")
    parser.add_argument('--modelpath', default=None, type=str,help='saved epoch num')
    parser.add_argument('--cmd_type', default="dbtest",dest='cmd_type', type=str,\
...
def test_img(args):
    ...
    Model = MobilePhoneNet(args)
    #Model = PhoneAttributeTF(args)
...
def evalue(args):
    ...
    for i in tqdm(range(total_num)):
        ...
        img_name = item_spl[0]
        real_label = int(item_spl[1]) # item_spl[1:] for sigmoid
        real_label = OneHotlabel(real_label,cfg.CLS_NUM) # for test softmax
        ...
        pred_ids = OneHotlabel(pred_ids,cfg.CLS_NUM) #for test softmax pred
        scores = probility[0]
        #print(pred_id.shape,probility.shape)
        tmp_item = dict()
        key_id = 0
