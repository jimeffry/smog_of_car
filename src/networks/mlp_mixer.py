...
from utils_plus import build_model_with_cfg
...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 2, 'input_size': (3, 448, 448), 'pool_size': None,
...
class MixerBlock(nn.Module):
    ...
    def __init__(
            ...
            mlp_layer=Mlp, norm_layer=partial(nn.LayerNorm, eps=1e-6), act_layer=nn.functional.gelu, drop=0., drop_path=0.):
        super().__init__()
    ...
    def forward(self, x):
        ...
        #print("token dim out",x.size())
        x = x + self.drop_path(self.mlp_channels(self.norm2(x)))
        # print("channel dim out",x.size())
        return x
...
class MlpMixer(nn.Module):
    ...
    def forward(self, x):
        x = self.stem(x)
        # print("embding:",x.size())
        x = self.blocks(x)
        x = self.norm(x)
        x = x.mean(dim=1)
        # print("logits:",x.size())
        x = self.head(x)
        # print("out",x.size(),self.head.weight.size())
        return x
...
def _create_mixer(variant, pretrained=False,num_classes=1000,img_size=[224,224], default_cfg=None, **kwargs):
...
if __name__=='__main__':
    net = mixer_l16_224_in21k(pretrained=True,num_classes=2)
    a=net(torch.ones((1,3,224,224)))