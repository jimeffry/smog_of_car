...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.layer5 = nn.Sequential(
                nn.Conv2d(512 * block.expansion, cfg.MODEL_ASPP_OUTDIM, 1, 1, padding=0, dilation=1,bias=True),
                # SynchronizedBatchNorm2d(dim_out, momentum=bn_mom),
                nn.BatchNorm2d(cfg.MODEL_ASPP_OUTDIM),
                nn.ReLU(inplace=True),
        )
        self.upsample_sub = nn.Upsample(scale_factor=4)
        self.shortcut_conv = nn.Sequential(
                nn.Conv2d(256, cfg.MODEL_SHORTCUT_DIM, 1, 1, padding=0,bias=True),
        ...
        self.fc = nn.Linear(cfg.MODEL_ASPP_OUTDIM, num_classes)
    ...
    def _forward_impl(self, x):
        ...
        x = self.layer4(x)
        x = self.layer5(x)
        x = self.upsample_sub(x)
        x1 = self.shortcut_conv(x1)
        feature_cat = torch.cat([x,x1],1)
        logits = self.cat_conv(feature_cat)
...
def resnet_aspp18(pretrained=False, progress=True, **kwargs):
...
def resnet_aspp34(pretrained=False, progress=True, **kwargs):
...
def resnet_aspp50(pretrained=False, progress=True, **kwargs):
...
def resnet_aspp101(pretrained=False, progress=True, **kwargs):
...
def resnet_aspp152(pretrained=False, progress=True, **kwargs):
